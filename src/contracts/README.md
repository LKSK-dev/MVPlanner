...
| 1.8.0   | Refactor: canonical `ConfirmFn` type (the `UiRegistry.confirm` shape) — replaces 10 duplicated per-module aliases. Type-only, additive.                                                                                                                                                                                                                    | orchestrator |
| 1.7.0   | UI remake: `PanelDef.meta?: WidgetMeta` (icon/category/singleton/defaultSize/settingsSchema) + `WidgetSettingField`; `PanelApi.settings?`/`onSettingsChange?` for per-instance widget config. All additive/optional — existing panels mount unchanged.                                                                                                     | orchestrator |
| 1.6.0   | App Settings 0.3: `AppearanceColorKey += 'outline'`; `AppearanceSettings += themeLibrary?/activeThemeId?` (+ `InstalledTheme`/`StoredThemeBundle`); `AppSettings += unitPreferences?` (+ `UnitPreferences`). All additive/optional.                                                                                                                        | orchestrator |
